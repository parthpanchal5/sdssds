      ...
      <div class="card-content">
        <table class="highlight responsive-table black-text animated fadeIn striped">
          ...
          <tbody>
            ...
            <?php $sql = "SELECT * FROM contact_us ORDER BY `contact_id` DESC LIMIT " . $startingLimitNo . ',' . $resultPerPage;
            $result = mysqli_query($conn, $sql);
            $checkRecord = mysqli_num_rows($result);
            if ($checkRecord < 1) {
              echo '<tr><td colspan="4">Your Inbox is empty!</td></tr>';
            }
            while ($row = mysqli_fetch_array($result)) { ?>
              ...
              <tr id="contact-row">
                ...
                <td><?php $date = date('d-M-Y', strtotime($row[3]));
                    echo $date; ?></td>
                <td>
